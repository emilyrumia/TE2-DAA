...
def knapsackbnb(W, val, wt):
    ...
    barang.sort(key=lambda x: x[0] / x[1], reverse=True)

    x = [0 for _ in range(len(barang))]
    x_topi = [0 for _ in range(len(barang))]
    i = 0
    z_topi = 0

    # Initialize empty sparse matrix M.
    M = [[0 for _ in range(W + 1)] for _ in range(len(barang))]

    x[0] = math.floor(W / barang[0][1])
    V_N = barang[0][0] * x[0]
    W_1 = W - barang[0][1] * x[0]
...
def develop(M, barang, x, i, V_N, W_1, U, m, z_topi, x_topi):
    if W_1 < m[i]:
        ...
        min_j = find_min_j(barang, W_1, i)
        if (V_N + cal_upper_bound(barang, W_1, V_N, min_j) <= z_topi) or (M[i][W_1] >= V_N):
            # Go to Step 3
            return M, barang, x, i, V_N, W_1, 3, z_topi, x_topi
        x[min_j] = math.floor(W_1 / barang[min_j][1])
        V_N += barang[min_j][0] * x[min_j]
        W_1 -= barang[min_j][1] * x[min_j]
        M[i][W_1] = V_N
        i = min_j
        # Go to Step 2
